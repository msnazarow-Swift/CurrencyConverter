...
class MainPresenter : MainPresenterProtocol{
  ...
  func setNewConvertedMoney(){
    guard let input = StorageService.shared.savedInputCurrency(),
          let output = StorageService.shared.savedOutputCurrency(),
          let amount = StorageService.shared.savedInputValue()
    else {
      return
    }
    NetworkManager.shared.getConvertion(from: input,  to: output ) { convertion in
